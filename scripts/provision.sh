...
# Name of the LWWSet cluster network
# connecting the peers
network="lwwset_network"
...
echo "Provisioning LWWSet Docker Cluster"

echo "Building LWWSet Docker Image"
docker build -t lwwset -f Dockerfile .

if [[ $? -ne 0 ]]; then
    echo "Unable To Build LWWSet Docker Image"
    exit 255
fi

echo "Building LWWSet Cluster Network"
...
for peer_index in "${!peers[@]}"; do
    docker run -p "${peers[$peer_index]}":8080 --net $network -e "PEERS="$comma_separated_peer_id_list"" -e "NETWORK="$network"" --name="peer-$peer_index" -d lwwset
done
